# Routing
Octobox uses a filesystem router built on top of React Location. If you have the patience, we recommend checking out [React Location's documentation](https://react-location.tanstack.com/overview) as well as this to get a gist of the internals of Octobox's router. It isn't necessary since you probably won't have to touch the internals, but it's nice to have background information.
...
With that, Octobox's route components (the actual components that render inside an outlet), are called windows. Each window's source code exists somewhere inside the `/src/windows/` directory. The paths of windows inside this directory define their URI and the windows themselves are named Window.tsx, similar to index.html. Windows also have different functions. There are 6 different types of windows:
...
### Static windows
Static windows are windows whose URIs on the server match their paths exactly, just like index.html files. For example:
```
...
/src/windows/shop/$product/$detail/Window.tsx -> /shop/car/price, /shop/car/specs, /shop/bike/deraiuler, /shop/bike/wheel, etc.
...
### Wildcard windows
Wildcard windows are like 404 pages; any and all windows at the same depth or deeper than their location which are not static windows will resolve. When both a dynamic and wildcard window exist in the same directory, the dynamic window will take priority and be resolved instead of the wildcard. Wildcard windows are denoted by being named `$wildcard`. For example:
```
...
### Error windows
Error windows are similar to pending windows, but they only render when a window fails to load or throws an exception. Octobox contains a global error window. Just like pending windows, any error windows in your app will override this global window for windows at the same depth or deeper than their location. They are denoted by a `$error` path segment.
```
...
You also cannot start static path segments with a `$`. If you need to use a dollar sign in a static window, you can create a dynamic window, write some code in the window to check to see if the current path matches the static path you wanted to create, and render the content inside the window if so.

Also, while this is not technically a reserved word, it's recommended to avoid having duplicate dynamic routes in one path such as `/$comment/$reply/$reply/$reply/...`. It's technically possible, but may make things a bit wonky.
...
* `pendingMs`
  ...
  * By default, this is 100 milliseconds before subtracting the unresponsive time&mdash;if the unresponsive time is default, the pending UI will be forced to be loaded for 500 milliseconds.
  * Set to 0 to disable.
* `maxAge`
...
### Data Unloading
Octobox also comes with a data unloading feature. When the router cache is cleared (when the maximum age is reached), Octobox will call any data unloaders. This allows data to be unloaded in a custom manner, letting you do any last-minute computations, clean up side effects, etc. They should be named exports with of the name `Unloader` and of the type `WindowUnloader`. For example:
```tsx
export const Unloader: WindowUnloader = async () => {
  // ...
};
```
`WindowUnloader`s are void-returning asynchronous functions, and cannot depend on a parent unloader like synchronous loaders can.

## Metadata Management
Octobox has a system to manage metadata, or the content of the document's `<head/>`. To do this, simply return a `metadata` paramater in a data loader. When the window renders, Octobox will update the document head. The `metadata` key is a reserved key of the type `MetaTags` in data loaders, and thus cannot be used to return actual data&mdash;they can only contain metadata. Their data also cannot be accessed by synchronous child loaders nor via the `useLoader()` hook.\
For example:
```tsx
export const Loader: WindowLoader = async () => {
  return {
...
> **Note**\
> Link tags and meta tags with the same keys won't override eachother. Only tags returned by children with the same keys and types as tags returned by parents will be overriden.
...
#### Stateful Anchors
As you might have noticed, the anchor example includes two child components of the `<Anchor/>`: an `<ActiveChild/>` and `<InactiveChild/>`. These add state to anchors, allowing you to, for example, style them differently if you're at the window they link to.
...
You can also use the `useAnchorState()` hook to programmatically determine whether a parent anchor is active or not.
...
### Loader Data
Data is returned by loaders via objects of the `PermissiveObjectWithMetadataType` and can be accessed in windows via `useLoader()`. The output from `useLoader()` is untyped, so you'll have to cast your data to the correct types.
...
## Reloading
You can reload the page programmatically using the `useReload()` hook. Static anchors also always reload the page.
...
```velocity
#set($nameOfElement = ${StringUtils.sub(${NAME}, "[...", "")})
#set($nameOfElement = ${StringUtils.sub(${nameOfElement}, "[", "")})
#set($nameOfElement = ${StringUtils.sub(${nameOfElement}, "]", "")})
